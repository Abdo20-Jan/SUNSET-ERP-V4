...
// biome-ignore lint/suspicious/noExplicitAny: serializador genérico de snapshots.
type GoldenJson = any;
...
export function serializeGolden(value: unknown): GoldenJson {
  if (value === null || value === undefined) return null;
  if (typeof value !== "object") return value;
  if (value instanceof Map) {
    const out: Record<string, GoldenJson> = {};
    for (const [k, v] of value.entries()) out[String(k)] = serializeGolden(v);
    return out;
  }
  if (Array.isArray(value)) return value.map(serializeGolden);
