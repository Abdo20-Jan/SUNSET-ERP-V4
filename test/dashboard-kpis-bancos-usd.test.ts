...
// Rollout USD del dashboard — el KPI "Saldo Bancos + Caja"
// (`getKpisPrincipales.saldoBancosCaja`) debe reconciliarse con el card
// "Saldos por Banco/Caja" (`getSaldosBancarios`). Antes el KPI sumaba ARS crudo
// del ledger sobre 1.1.1.*/1.1.2.* (sin filtrar activa) → divergía del card por:
//   (1) moneda: USD al TC histórico ÷ TC ≠ saldo USD nativo,
//   (2) incluía 1.1.2.* (inversiones financieras) y cuentas inactivas.
// Ahora el KPI deriva de la MISMA fuente del card y expone {ars, usd} nativo.
...
import { getKpisPrincipales, getSaldosBancarios } from "@/lib/services/dashboard";
...
describe("getKpisPrincipales — saldoBancosCaja native-aware", () => {
  it("expone {ars, usd} nativo y reconcilia con el card (getSaldosBancarios)", async () => {
    ...
    expect(kpis.saldoBancosCaja.ars.toFixed(2)).toBe("500000.00");
    expect(kpis.saldoBancosCaja.usd.toFixed(2)).toBe("7000.00");

    // El card sólo trae caja (ARS) + banco USD (activos, 1.1.1.01/02);
    // inversión 1.1.2 e inactiva quedan fuera.
    ...
    const cardUsd = saldos
      ...
      .reduce((acc, s) => acc + Number(s.saldo), 0);
    expect(kpis.saldoBancosCaja.ars.toFixed(2)).toBe(cardArs.toFixed(2));
    expect(kpis.saldoBancosCaja.usd.toFixed(2)).toBe(cardUsd.toFixed(2));
  });

  it("sin cuentas de banco/caja: {ars:0, usd:0}", async () => {
    const kpis = await getKpisPrincipales();
    expect(kpis.saldoBancosCaja.ars.toFixed(2)).toBe("0.00");
    expect(kpis.saldoBancosCaja.usd.toFixed(2)).toBe("0.00");
  });
});
