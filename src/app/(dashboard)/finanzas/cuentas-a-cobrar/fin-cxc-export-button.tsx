...
/**
 * Botón de exportación AUDITADA de la worklist de gestión de cuentas a cobrar
 * (FIN-01 · PR-026). Espejo de `cuentas-a-cobrar-export-button.tsx`/PR-025c:
 * toma `vista`/`moneda` de la URL y delega en la server action
 * `exportarFinCxc` (que re-lee server-side con los MISMOS presets, re-chequea
...
export function FinCxcExportButton() {
  ...
  const run = (formato: "csv" | "xlsx") => {
    ...
    start(async () => {
      const res = await exportarFinCxc({ params, formato });
      if (res.ok) {
