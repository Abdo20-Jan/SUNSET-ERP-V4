...
/*
 * MovimientoDetalleWorkWindow (TES-01 · PR-025a) — migra el drawer lateral
 * `movimiento-detalle-sheet.tsx` a FloatingWorkWindow (G-04). Read-only: SÓLO
 * cambia el contenedor (Sheet → FWW). El body (datos del movimiento + asiento
 * generado) y el fetch `getAsientoDetalle` (CALL, no se modifica) son idénticos.
 */

type Props = {
  movimiento: MovimientoWorklistRow | null;
  open: boolean;
...
export function MovimientoDetalleWorkWindow({ movimiento, open, onOpenChange, moneda, tc }: Props) {
    ...
    <FloatingWorkWindow
      ...
      title={
        <span className="flex items-center gap-2">
          <span>{movimiento ? `Movimiento ${movimiento.tipo}` : "Movimiento"}</span>
          {movimiento?.asiento && (
            <>
      ...
      description={movimiento?.descripcion ?? "Detalle del movimiento y asiento generado."}
      initialWidth={720}
      initialHeight={640}
    >
...
function ContrapartidaRow({ movimiento }: { movimiento: MovimientoWorklistRow }) {
  return (
    <div className="flex flex-col">
