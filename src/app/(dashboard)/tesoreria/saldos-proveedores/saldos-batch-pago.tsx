...
export function SaldosBatchPago({ proveedores, cuentasBancarias, defaultFecha }: Props) {
  ...
  const allSelectableIds = proveedores
    ...
    .map((p) => p.proveedorId);
  const allSelected = selected.size === allSelectableIds.length && allSelectableIds.length > 0;

  return (
    <>
      <Card className="py-0">
        <Table>
          <caption className="sr-only">Saldos por proveedor con desglose de vencimientos</caption>
          <TableHeader>
            <TableRow>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex flex-col">
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  {seleccionados.length === 1 ? "" : "s"} (subtotal facturas)
                </span>
            ...
            <label className="flex cursor-pointer items-start gap-2 rounded-md border bg-card px-3 py-2 text-[12px]">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-muted-foreground">
                  Activá si transferís a un despachante (ej: CYSAR) que paga las facturas a
                  TRP/EXOLGAN/etc en tu nombre. La diferencia entre el monto transferido y el
                  subtotal de facturas queda como anticipo (a tu favor) o saldo pendiente con el
                  intermediário.
                </span>
              </div>
            </label>
...
function ProveedorRow({
  ...
  return (
    <>
      <TableRow
        className={
          isChecked ? "bg-primary/5" : tieneVencidas ? "bg-red-50/40 dark:bg-red-950/10" : undefined
        }
      >
        <TableCell>
          <Checkbox
