...
import type { VepDespachoPendiente } from "./vep-despacho-section";

/*
 * PagarVepDespachoWorkWindow (TES-02 · PR-025b-2) — migra el dialog
 * `PagarVepDespachoDialog` de `vep-despacho-section.tsx` (mantido en árbol como
 * dead export — rollback) a FloatingWorkWindow (G-04). SÓLO cambia el
 * contenedor (Dialog → FWW; el header pasa a los props `title`/`description`;
 * el `DialogClose` pasa a `Button onClick={onClose}`): el body (form completo,
 * crédito Aduana, validaciones, toasts) y la action `pagarVepDespachoAction`
 * (CALL, no se modifica) son idénticos — payload byte-idéntico.
 */

export function PagarVepDespachoWorkWindow({
  vep,
  cuentasBancarias,
  saldoCreditoAduana,
  onClose,
  ...
  return (
    <FloatingWorkWindow
      ...
      title={<>Pagar VEP del despacho {vep.despachoCodigo}</>}
      description={
        <>
