...
/*
 * PrestamoDetalleWorkWindow (TES-02 · PR-025b) — migra el drawer lateral
 * `prestamo-detalle-sheet.tsx` (mantido en árbol, no importado — rollback) a
 * FloatingWorkWindow (G-04). Read-only: SÓLO cambia el contenedor (Sheet → FWW;
 * el header pasa a los props `title`/`description`). El body (datos del
 * préstamo + asiento de recepción + amortizaciones) y el fetch
 * `obtenerPrestamoDetalle` (CALL, no se modifica) son idénticos.
 */
...
export function PrestamoDetalleWorkWindow({ prestamoId, open, onOpenChange, moneda, tc }: Props) {
    ...
    <FloatingWorkWindow
      ...
      title={
        <span className="flex items-center gap-2">
          <span>{detalle ? `Préstamo · ${detalle.prestamista}` : "Préstamo"}</span>
          {detalle?.asiento && (
            <>
      ...
      description="Detalle del préstamo, asiento de recepción y amortizaciones registradas."
      initialWidth={720}
      initialHeight={640}
    >
