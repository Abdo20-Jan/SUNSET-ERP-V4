...
/*
 * AnticipoDetalleWorkWindow (TES-02 · PR-025b) — migra el drawer lateral
 * `anticipo-detalle-sheet.tsx` (mantido en árbol, no importado — rollback) a
 * FloatingWorkWindow (G-04). SÓLO cambia el contenedor (Sheet → FWW; el header
 * pasa a los props `title`/`description`): el body (datos + aplicaciones +
 * form "Aplicar a factura" + Anular) y las actions
 * `getAnticipoDetalle`/`listarFacturasAplicablesProveedor`/
 * `aplicarAnticipoProveedorAction`/`anularAnticipoProveedorAction` (CALL, no se
 * modifican) son idénticos — payloads byte-idénticos. El Dialog de confirmación
 * de anular pasa a HERMANO de la FWW (patrón nueva-cuenta-work-window).
 */
...
export function AnticipoDetalleWorkWindow({
  anticipoId,
  proveedorId,
  open,
  onOpenChange,
      ...
      <FloatingWorkWindow
        ...
        title={
          <span className="flex items-center gap-2">
            <span>{detalle ? `Anticipo ${detalle.numero}` : "Anticipo"}</span>
            {detalle && (
        ...
        description="Detalle del anticipo, aplicaciones a facturas y acciones disponibles."
        initialWidth={720}
        initialHeight={640}
      >
