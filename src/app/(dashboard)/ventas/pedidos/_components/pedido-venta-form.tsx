...
export function PedidoVentaForm({
  ...
  const submit = handleSubmit((values) => {
    startTransition(async () => {
      const result = await guardarPedidoVentaAction({
    ...
    <form onSubmit={submit} className="flex flex-col gap-6 pb-32">
      ...
      <Card>
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-xs uppercase tracking-wide">Cliente</Label>
            <Controller
              control={control}
              name="clienteId"
              render={({ field }) => (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Sin IVA acá. El IVA se calcula al crear la factura.
              </p>
            </div>
            <Button
              type="button"
