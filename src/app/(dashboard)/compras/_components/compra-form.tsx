...
export function CompraForm({
  ...
  const submitGuardar = handleSubmit((values) => {
    startTransition(async () => {
      const result = await guardarCompraAction(buildPayload(values));
      if (result.ok) {
        toast.success(`Compra ${result.numero} guardada (BORRADOR).`);
  ...
  const submitEmitir = handleSubmit((values) => {
    startTransition(async () => {
      const saved = await guardarCompraAction(buildPayload(values));
      if (!saved.ok) {
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          {isEdit
            ...
            : "Registre la compra en BORRADOR. Al emitir, se genera el asiento contable."}
        </p>
      </div>
      ...
      <Card>
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Field label="Número" error={errors.numero?.message}>
            <Input {...register("numero")} placeholder="C-2026-0001" />
          </Field>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Cada ítem se valoriza en moneda de la compra.
              </p>
            </div>
