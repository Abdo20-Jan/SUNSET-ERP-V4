...
export function PedidoCompraDetail({
  pedido,
  proveedorNombre,
  productosMap,
  comprasVinculadas,
  ...
  const handleTransicion = (estado: PedidoEstado) => {
    startTransition(async () => {
      const res = await transicionarPedidoCompraAction(pedido.id, estado);
      if (res.ok) {
  ...
  const handleCrearFactura = () => {
    startTransition(async () => {
      const res = await crearCompraDesdePedidoAction(pedido.id);
      if (res.ok) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            {proveedorNombre} · {fmtDate(new Date(pedido.fecha))} · {pedido.items.length} ítem
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {editable && (
            <Link
              href={`/compras/pedidos/${pedido.id}?editar=1`}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              El pedido pasará a CANCELADO y ya no podrá facturarse. Las facturas ya creadas siguen
              vigentes (pueden anularse desde su vista propia si corresponde).
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
