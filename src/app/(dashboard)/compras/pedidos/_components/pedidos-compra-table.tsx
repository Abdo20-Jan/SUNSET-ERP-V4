...
export function PedidosCompraTable({ data }: { data: PedidoCompraRow[] }) {
  const columns: ColumnDef<PedidoCompraRow>[] = [
    {
      id: "numero",
      header: "Número",
      cell: ({ row }) => (
        <Link
          href={`/compras/pedidos/${row.original.id}`}
  ...
  return (
    <Table>
      <caption className="sr-only">Pedidos de compra (OC) registrados</caption>
      <TableHeader>
